...
# Marchenko-Pastur initialization
...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        if global_model_type == 'None':
            ...
            mp_init_weight(self.self_attn.in_proj_weight)
            if self.self_attn.out_proj is not None:
                mp_init_weight(self.self_attn.out_proj.weight)
        elif global_model_type == 'Performer':
            self.self_attn = SelfAttention(
                dim=dim_h, heads=num_heads,
                dropout=self.attn_dropout, causal=False)
            # MP init for Performer SelfAttention weights
            if hasattr(self.self_attn, 'to_qkv'):
                mp_init_weight(self.self_attn.to_qkv.weight)
            if hasattr(self.self_attn, 'to_out'):
                mp_init_weight(self.self_attn.to_out.weight)
        elif global_model_type == "BigBird":
            bigbird_cfg.dim_hidden = dim_h
            bigbird_cfg.n_heads = num_heads
            bigbird_cfg.dropout = dropout
            self.self_attn = SingleBigBirdLayer(bigbird_cfg)
            # Optionally, add MP init for BigBird if it has linear weights
        else:
